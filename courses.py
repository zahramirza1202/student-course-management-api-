...
#DELETE /courses/<id> -> delete course and their description
...
def delete_course(course_id):
    ...
    if not cursor.fetchone():
        ...
        return jsonify({"error":"Course not found"}), 404
    try:
        #delete related enrollments first to msintain referential integrity
        cursor.execute("DELETE FROM enrollments WHERE course_id =%s",(course_id,))
        #Delete the course
        cursor.execute("DELETE FROM courses WHERE id=%s",(course_id,))
        conn.commit()
        return jsonify({"message":"Course and related enrollment deleted successfully"}), 200
